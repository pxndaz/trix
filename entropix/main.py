...
from entropix.stats import AttnStats  # Importing the updated AttnStats with varentropy detection
...
def main(weights_path: Path = DEFAULT_WEIGHTS_PATH.joinpath('1B-Instruct')):
    ...
    def generate(xfmr_weights, model_params, tokens):
        ...
        # Initialize AttnStats to track entropy and varentropy
        stats = AttnStats.new(bsz, model_params.n_layers, model_params.n_heads)
        consecutive_jumps = 0  # Counter for varentropy jumps
        ...
        while cur_pos < 8192:
            ...
            # Update AttnStats and check for varentropy jumps
            stats = stats.update(scores, layer_idx=cur_pos % model_params.n_layers)
            if stats.prev_varentropy[:, :, -1] > stats.prev_varentropy[:, :, -2]:  # Check if varentropy is increasing
                consecutive_jumps += 1
            else:
                consecutive_jumps = 0  # Reset counter if no jump detected

            if consecutive_jumps >= 3:  # Trigger feedback after 3 jumps
                print("\n--- Triggering Feedback ---")
                break  # or call your feedback function here
